...
int createSem() {
    ...
    if ((semid = semget(sem_key++, 2, 0666 | IPC_CREAT)) < 0) {
        printf("Unable to create semaphore set\n");
        exit(-1);
    }

    printf("Semaphore set created with identifier: %d\n", semid);
...
void deleteSem(int semid) {
    if (semctl(semid, 0, IPC_RMID) < 0) {
        printf("Unable to delete semaphore set\n");
        exit(-1);
    }
}
    ...
    if (writer_process < 0) {
        ...
        for (int i = 0; i < amount_of_operations; i++) {
            ...
            if (semop(reader_sem_id, &semaphore_operation, 1) < 0) {
                printf("Unable to wait for condition\n");
                exit(-1);
            }
        ...
        printf("Writer completed its work\n");
    } else {
        for (int i = 0; i < amount_of_operations; i++) {
            semaphore_operation = setSemOperation(0, -1, 0);
            if (semop(reader_sem_id, &semaphore_operation, 1) < 0) {
        ...
        printf("Reader completed its work\n");
    }
